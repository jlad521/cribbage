...
clientCribbage::clientCribbage(){
  try {
        ...
        myClient.call(serverUrl,getGameInfo, &result);
...
void clientCribbage::playRound(Player* players[] , ShowCribbage* display) {
    srand(time(NULL));
    Deck * deck = new Deck();
...
void clientCribbage::goPhase(Player* players[], Deck * deck, ShowCribbage* display, Card* cut){
    pTurn = (dealerPos +1) % 2;
...
int clientCribbage::goPoints(vector<Card*> v){
    int pts = 0;
    pts += PairsGo(v);
    pts += RunScoreGo(v);
    return pts;
...
int clientCribbage::AIGoChoice(Player* p, int goNum){
...
int clientCribbage::scorePhase(vector<Card*> hand, Card* cut){
    int points = 0;
    points += Pairs(hand);
...
vector<Card*> clientCribbage::sortHand(vector<Card*> v){
    int i, j;
    Card*  t;
    vector<Card*> temp;
...
int clientCribbage::JackSameSuit(vector<Card*> v, Card* cut){ // 1 pt if player's card is Jack and same suit as starter card
    int i;
...
int clientCribbage::Pairs(vector<Card*> v){ // 2 pts per pair (3 of a kind - 3 pairs; 4 of a kind - 6 pairs)
    int n = 0;
    int i, b;
...
int clientCribbage::ThirtyOne(vector<Card*> v){ // 2 pts for reaching 31 exactly
    int n = 0;
    int i;
...
int clientCribbage::Fifteen(vector<Card*> v){  // 2 pts per combination that equals fifteen
    int n = 0;
    int i = 0;
    int b, d, e;
    vector<Card*> temp;
...
int clientCribbage::RunScore(vector<Card*> v){ // n points per card in run
    v = sortHand(v);
...
int clientCribbage::FlushScore(vector<Card*> v){ // 4 points for 4 cards in player's hand all of same suit
    int i = 0;
    int clubs = 0;
    int diamonds = 0;
    int hearts = 0;
...
int clientCribbage::PairsGo(vector<Card*> v){
    int SIZE = (v.size()-1);
...
int clientCribbage::RunScoreGo(vector<Card*> v){
    int SIZE = (v.size()-1);
    /*
       if(SIZE == 3){
